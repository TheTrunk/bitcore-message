...
Message.MAGIC_BYTES = new Buffer('Hush Signed Message:\n');
...
/**
 * Will sign a message with a given Hush private key.
 *
...
/**
 * Will return a boolean of the signature is valid for a given Hush address.
 * If it isn't the specific reason is accessible via the "error" member.
 *
 * @param {Address|String} hushAddress - A Hush address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(hushAddress, signatureString) {
  $.checkArgument(hushAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(hushAddress)) {
    hushAddress = Address.fromString(hushAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, hushAddress.network);

  // check that the recovered address and specified address match
  if (hushAddress.toString() !== signatureAddress.toString()) {
